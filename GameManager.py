...
from State.QuestState import Quest, NotStarted, InProgress, Completed
...
class GameManager:
    ...
    def execute_combat_turn(self, action="attack", strategy="balanced"):
        ...
        if action == "attack":
            ...
            health_potions = [item for item in self.inventory["items"] 
                             if item.get("name") == "Health Potion"]
            if health_potions:
                potion = health_potions[0]
                self.player["pv"] += potion.get("value", 50)
                self.inventory["items"].remove(potion)
                print(f"{self.player['name']} used Health Potion! (+{potion.get('value', 50)} HP)")
            else:
        ...
        if self.player["pv"] <= 0:
            ...
            if self.current_combat.enemy.get("type") == "boss":
                ...
                if self.game_progress["bosses_defeated"] >= 1:
                    self.dungeon_event.enemy_defeated(self.game_progress["bosses_defeated"])
    ...
    def get_game_state_summary(self):
        ...
        return {
            ...
            "current_area": self.exploration_system.get_current_area()["name"] if self.exploration_system else "None",
            "inventory_items": len(self.inventory["items"]),
            "active_quests": len([q for q in self.quests if q.get_status() == "In Progress"]),
            "completed_quests": len([q for q in self.quests if q.get_status() == "Completed"]),
    ...
    def display_game_state(self):
        ...
        print(f"Active Quests: {len([q for q in self.quests if q.get_status() == 'In Progress'])}")
