...
class AggressiveStrategy(BattleStrategy):
    def execute_attack(self, attacker, defender):
        damage = attacker.get("offensive_stat_2", attacker.get("offensive_stat", 0))
        ...
        print(f"{attacker['name']} uses aggressive strategy!")
        print(
            f"{attacker['name']} attacks {defender['name']} for {final_damage} damage!"
        )
        return final_damage
